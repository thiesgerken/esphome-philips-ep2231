...
- the checksum is understood well enough to compute any message (see [protocol.md](protocol.md)); incoming frames are still validated by comparing the trailing two bytes against the previous message, which works because the mainboard repeats every frame
